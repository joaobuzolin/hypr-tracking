...
BEGIN
  ...
  -- Find which campaign_ids WERE in the materialized view
  SELECT ARRAY_AGG(cms2.campaign_id) INTO found_ids
  FROM campaign_metrics_summary cms2
  WHERE cms2.campaign_id = ANY(campaign_ids);

  -- Compute missing IDs
  SELECT ARRAY_AGG(uid) INTO missing_ids
  FROM unnest(campaign_ids) AS uid
  WHERE uid != ALL(COALESCE(found_ids, ARRAY[]::uuid[]));
